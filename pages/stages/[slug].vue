...
const item = stageItems.find((entry) => entry.slug === route.params.slug);
...
const localizedItem = computed(() => localizeStageItem(item, locale.value));
...
<template>
  <article class="mx-auto max-w-4xl space-y-8">
    <NuxtLink to="/stages" class="inline-flex rounded-full border border-pine/15 bg-white px-4 py-2 text-sm font-semibold text-pine">
      {{ copy.common.backToStages }}
    </NuxtLink>
    <section class="rounded-[2rem] bg-white p-8 shadow-float">
      <div class="flex flex-wrap items-center gap-3">
        <span class="rounded-full bg-mint px-3 py-1 text-xs font-semibold text-pine">{{ localizedItem.category }}</span>
    ...
    <section class="rounded-[2rem] bg-white p-8 shadow-float">
      <h2 class="text-2xl font-semibold text-pine">{{ copy.common.stageIntro }}</h2>
      <p class="mt-4 text-base leading-8 text-ink/75">{{ localizedItem.intro }}</p>
      <div class="mt-6 rounded-[1.5rem] bg-sand/60 p-6">
    ...
    <section class="rounded-[2rem] bg-white p-8 shadow-float">
      <h2 class="text-2xl font-semibold text-pine">{{ copy.common.stageFocus }}</h2>
